...
void build_board() {
    ...
    clk_create_cron(&game_time, 10, 30, 0, 0, MEDIUM_FONT, c_orange, BACK_COLOR);
    ct_create(&counter, 560, 30, 0, "", MEDIUM_FONT);
...
Location graph_to_board_loc(int x, int y) {
	...
	if (x < (BOARD_X + CARD_BORDER_X ) || x >= BOARD_X + BOARD_WIDTH - CARD_BORDER_X ||
	    y <= BOARD_Y - BOARD_HEIGHT + CARD_BORDER_Y || y > BOARD_Y - CARD_BORDER_Y) {
		  l.line = l.col = -1;
	}
	else {
		l.col = (x- BOARD_X)/(CARD_BORDER_X+CARD_WIDTH);
		l.line = (BOARD_Y - y) /(CARD_BORDER_Y+CARD_HEIGHT);
		if (l.col >= NCOLS || l.col < 0 || l.line >= NLINES || l.line < 0)
...
void draw_board() {
	graph_rect(0,0,800, 600, BACK_COLOR, true);
	for (int l=0; l < NLINES; ++l) {
		for(int c=0; c < NCOLS; ++c) {
			 show_card(l,c);
		}
